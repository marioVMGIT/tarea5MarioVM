...
function frutaCompr(expresion) {
    switch (expresion) {
        ...
        case "platanos":
            ...
            break;
        case "cerezas":
            document.precioFruta.texto.value = "Las cerezas cuestan 750 colones";
            break;
        case "mangos":
        case "papayas":
...
//Error
try {
    throw new Error("¡Ups - ERROR GENERADO PRUEBA!");
} catch (e) {
...
function ejmFunctionPotencia(number) {
    if (isNaN(number))
        ...
        document.potenciaCalc.potenciaNumber.value = "El resultado es " + Math.pow(number, 2);
}
